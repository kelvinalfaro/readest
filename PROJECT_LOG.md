...
- Validation passed TypeScript/Biome lint across 2,541 files, repository formatting across 2,576 files, 125 focused web tests, native TTS Kotlin tests, and a complete arm64 debug Android build. The full web suite passed 12,015 tests with 16 skipped; timing-sensitive native-app module-loading tests passed on isolated rerun and received explicit 15-second limits so the full pre-push gate does not fail under parallel load. The version assertion updated for the new scheme also passed on rerun. The built APK reports package `com.bilingify.readest.cwa`, version name `0.12.10-cwa.1`, version code `12026`, and only the arm64 Readest library.
