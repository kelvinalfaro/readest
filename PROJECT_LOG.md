...
- Validation passed TypeScript/Biome lint across 2,541 files, repository formatting across 2,576 files, 125 focused web tests, native TTS Kotlin tests, and a complete arm64 debug Android build. The pre-push full web suite passed 12,017 tests with 16 skipped after timing-sensitive native-app module-loading tests received explicit 15-second limits for parallel load. The version assertion updated for the new scheme also passed on rerun. The built APK reports package `com.bilingify.readest.cwa`, version name `0.12.10-cwa.1`, version code `12026`, and only the arm64 Readest library.
- Pushed merge and test-stability commits through `3471122e0`, then published signed dual-APK release [`Readest CWA Android 0.12.10-cwa.1`](https://github.com/kelvinalfaro/readest/releases/tag/cwa-android-v0.12.10-cwa.1-upstream-3471122) through successful workflow run [`35749446640`](https://github.com/kelvinalfaro/readest/actions/runs/35749446640). The workflow verified distinct phone arm64 and Android TV armv7 profiles. Shared updater metadata was deliberately disabled, so the release did not replace the current Latest release and requires a one-time manual install; physical device acceptance remains open.
- For upstream [issue #6353](https://github.com/readest/readest/issues/6353), the connected phone showed active Read Aloud with `launchIntent=null` in `dumpsys media_session`, and Kelvin confirmed that tapping the shade media widget did not open Readest. Opened [PR #6370](https://github.com/readest/readest/pull/6370) from a clean upstream-based branch to set the media session's launcher `PendingIntent`; it is ready for review. Kotlin plugin compilation/unit tests, frontend build, repository format/lint, and the full 11,949-test suite passed; a patched-device tap test remains open. Local full Android packaging stopped earlier in an unrelated Rust `turso_sdk_kit` linker step (`-ladvapi32`). Upstream CI passed all checks except one Chromium paginator animation test outside the diff; the focused browser file passed locally (35/35), and GitHub did not permit a contributor rerun of the upstream job. CodeRabbit found no actionable issues. Kelvin trimmed the PR description and will update it after phone testing.
- Ported the media-session launch intent fix into fork `main` at `b6a1dbda4` and dispatched the signed, artifact-only [CWA Android Build run 35776407189](https://github.com/kelvinalfaro/readest/actions/runs/35776407189) with `create_release=false`. Kotlin compilation/native unit tests, format/lint, and the full 12,017-test app suite passed before push. The workflow was in progress at handoff; Kelvin will monitor, install the APK artifact, and test that tapping the media widget reopens Readest. This build does not publish a release or update `latest.json`.
- Live device inspection showed the phone still had the earlier `0.12.10-cwa.1` release APK (`2f0627...`), not the same-version widget-fix artifact (`d8bf79...`); identical version name/code and the artifact-only workflow explained both the failed updater path and ambiguous manual install. During cold Android Auto playback, Android audio state and Kelvin's listening test confirmed native Google TTS continued after opening Readest started Edge/Aria, producing two slightly offset voices. Kelvin approved a uniquely versioned `0.12.10-cwa.2` / `12027` release with explicit native-to-WebView ownership transfer, an explicit launcher PendingIntent shared by the media session and notification, and updater metadata after validation. The focused Android declarations/ABI suite passed 25 tests, the arm64 release production Kotlin compilation passed, and the native-TTS release unit suite passed; the combined app-unit task remains independently blocked by the existing release-variant `KeyLearnCaptureTest` helper wiring.
- Pushed `82a3aa052` after the full pre-push gate passed format, lint, and 12,019 app tests with 16 skipped. Signed dual-APK [release `0.12.10-cwa.2`](https://github.com/kelvinalfaro/readest/releases/tag/cwa-android-v0.12.10-cwa.2-37) completed in [workflow run 35811465873](https://github.com/kelvinalfaro/readest/actions/runs/35811465873), verified distinct phone arm64 and TV armv7 profiles, and published `latest.json` with version `0.12.10-cwa.2`. Kelvin updated through the phone app; the device reports version code `12027`, the live Readest media session now has a non-null launcher PendingIntent, and tapping the shade media widget opens the app. Cold Android Auto book-start and Google-to-Aria no-overlap acceptance remain for Kelvin's next car test.
...
- Physical testing of `0.12.23` reached the correct selected EPUB and foreground service but reported `Read Aloud could not open this book`. The live native trace identified `ParserConfigurationException` for `disallow-doctype-decl`: Android's Harmony XML parser does not implement that desktop JAXP feature. Release `0.12.24` strips DOCTYPE declarations before parsing, applies supported external-entity flags best-effort, and installs an explicit blocking entity resolver. Kotlin compilation and native EPUB tests pass; Kelvin authorized the signed follow-up build for physical acceptance.
- Physical acceptance of `0.12.24` confirmed that cold Android Auto playback starts. Follow-up testing exposed three narrower defects: the cold EPUB reader used only the CFI spine step and therefore resumed at the beginning of the current chapter/section; the WebView takeover registered transport handlers after awaited native setup so the first Pause could update the car icon without stopping speech; and a cold `ACTION_MEDIA_BUTTON` was discarded instead of restarting the persisted last book.
- The current local repair resolves the saved content-document CFI down to its text offset, preserves that precise CFI while cold playback remains in the same section, installs pause/play handlers before native activation waits, registers native transport listeners before notification-permission work, and dispatches cold media-button intents through the media session. Validation passed 55 focused media tests, TypeScript, targeted Biome, isolated Kotlin EPUB tests, diff checks, and the full app suite (11,227 passed; 16 skipped). The generated Tauri Android support files are absent locally, so the signed workflow remains the native integration/build gate; no push, install, or release has been performed for this repair.
- Installed Visual Studio 2022 Build Tools with the MSVC C++ workload and verified Rust can discover `link.exe` from a normal PowerShell session. A local arm64 Android debug build then exposed and fixed two cross-platform build defects: `turso_ext 0.6.1` linked Windows `advapi32` based on the host instead of the Android target, and generated Tauri `buildSrc` Kotlin duplicated the maintained sources. The complete local build now succeeds and emits a verified `0.12.24` debug APK; it is debug-signed and therefore is not an update-install replacement for the signed release app. No APK was installed, and no push or release was performed.
- Prepared signed updater release `0.12.25` for the precise-CFI resume, first-command transport, and cold media-button repairs. Kelvin authorized pushing the validated source and publishing shared updater metadata, and will monitor the workflow, install the update, and report physical phone/car acceptance.
...
- Signed release `0.12.21` built successfully in GitHub Actions and published its APK plus `latest.json`. Physical testing confirmed that Android Auto now switches to the correct cover and follows phone-started audiobook sessions, but selecting an ebook while the phone app is cold still leaves the car at `0:00 / 0:00` with a nonfunctional Pause control. With the phone app open, the selection reaches the reader but the Read Aloud request does not start.
- Follow-up repair uses Android 16's `MODE_BACKGROUND_ACTIVITY_START_ALLOW_ALWAYS` for the connected-device cold-launch `PendingIntent` (with the older mode retained for Android 14–15) and moves one-shot Read Aloud consumption into the per-book TTS control. The request now remains pending until the view, progress, settings, and book data are all ready instead of being discarded by an early event. An early car-side Pause now cancels both native and WebView queues and returns the session to Paused; Play retries the selected/last book. Added native launch diagnostics for the next physical acceptance run.
...
- Focused Android Auto/library tests, TypeScript, targeted Biome checks, and the production frontend build passed. Signed CWA release `0.12.19` was installed and accepted on physical Android Auto: the launcher entry, library browsing, book selection/resume, Read Aloud playback, and cover artwork all work.
- Ported only the public Android Auto compatibility changes onto current upstream `main` at commit `6e6e165ac`, pushed branch `feat/android-auto-library` to the fork, and opened upstream PR [`readest/readest#6285`](https://github.com/readest/readest/pull/6285). The upstream-facing full gate passed 944 test files and 11,343 tests, plus repository-wide format, TypeScript, and Biome checks; upstream CI and review remain open.
- Follow-up device testing found two audiobook handoff races: the prior book's asynchronously decoded cover could overwrite the replacement session, and playback could begin before the media bridge finished registering listeners, leaving Android Auto stopped over live phone audio. Native media updates and teardown now carry a book-session identity so stale work is rejected; a new owner immediately clears/seeds its artwork, and bridge binding performs a catch-up playback/timeline write.
- Local validation for the handoff fix passed 51 focused media-session, bridge, and Android Auto tests; TypeScript/Biome lint across 2,410 files; repository formatting across 2,445 files; Rust formatting; and diff checks. Prepared patch release `0.12.20`; the generated Android Gradle project remains unavailable locally, so the signed workflow build and physical Android Auto switch test are still required.
- Updated upstream PR [`readest/readest#6285`](https://github.com/readest/readest/pull/6285) at commit `adec60f44`: the published car-library model now carries audiobook identity so cold audiobook selections route to the player instead of TTS autoplay, and the later session-ownership/artwork/playback catch-up protections were propagated without CWA release changes. Replied to and resolved the review thread. Validation passed 52 focused tests, TypeScript/Biome/format checks, and 11,350 tests across 944 full-suite files; GitHub CI on the new PR commit remains the native compile/review gate.
- A second review round identified stale asynchronous ownership races in listener teardown and TTS rebinding. PR commit `0f32be076` now detaches and cleans only the retiring session's listener set and prevents an obsolete bind from publishing the previous book's metadata or handlers. Both review threads were answered, resolved, and independently confirmed by CodeRabbit; 49 focused tests, TypeScript/Biome/format checks, and the full suite (11,351 tests across 944 files) passed. The same fix was pushed to CWA `main` at `1c751d532`; no new CWA release was created.
...
- Reconciled the upstream merge with retained CWA behavior: restored bounded native TTS initialization and full failed-start cleanup, kept foreground media activation ahead of asynchronous artwork, retained M4B selection alongside Android TV ZIP handling, preserved ungated offline audio, updated the Android updater mock interface, and made multi-provider sync aggregation tolerate partial results.
- Focused validation passed 129 tests across the previously failing updater, file-selection, TTS lifecycle/player, and library-sync files. Full TypeScript checking, Biome lint, production build, pre-push suite, native APK build, and route-handoff device verification are the remaining release gates.
