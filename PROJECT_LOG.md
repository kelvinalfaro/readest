...
- A second review round identified stale asynchronous ownership races in listener teardown and TTS rebinding. PR commit `0f32be076` now detaches and cleans only the retiring session's listener set and prevents an obsolete bind from publishing the previous book's metadata or handlers. Both review threads were answered and resolved; 49 focused tests, TypeScript/Biome/format checks, and the full suite (11,351 tests across 944 files) passed. The same fix is preserved locally on CWA `main` at `1c751d532`; no new CWA release was created.
