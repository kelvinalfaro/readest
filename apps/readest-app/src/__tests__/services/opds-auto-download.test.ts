...
describe('OPDS auto-download orchestrator', () => {
  ...
  it('keeps counting retry attempts for an entry in a later persist batch', async () => {
    ...
    const savedState = vi
      .mocked(saveSubscriptionState)
      .mock.calls.at(-1)![1] as OPDSSubscriptionState;
    // Third strike: marked known for automatic sync while retained for the
    // fork's explicit CWA/BookOrbit retry action.
    expect(savedState.failedEntries).toEqual([
      expect.objectContaining({ entryId: 'urn:shelf:doomed', attempts: MAX_RETRY_ATTEMPTS }),
    ]);
