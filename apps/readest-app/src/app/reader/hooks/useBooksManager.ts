...
const useBooksManager = () => {
  ...
  const openBookInReader = (bookHash: string, cfi?: string) => {
    ...
    if (existing) {
      ...
      // already mounted, and focusing it leaves bookKeys unchanged.
  ...
  // Consume an Android Auto cold-resume autoplay request once its book is in
  // the open set (covers both the in-place open and cold-navigate paths).
  useEffect(() => {
